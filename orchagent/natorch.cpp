...
#define NAT_MAX_CAPACITY_DEFAULT    1024
         ...
         m_twiceNaptQueryTable(appDb, APP_NAPT_TWICE_TABLE_NAME)
{
    ...
    maxAllowedNatEntries = NAT_MAX_CAPACITY_DEFAULT;
}
...
void NatOrch::removeDnatFromNhCache(const IpAddress &translatedIp, const IpAddress &dstIp)
{
    ...
    if (dnatEntries.dnapt.empty() && (dnatEntries.dnatIp == IpAddress()) &&
...
{
    std::string res;
    std::string cmd1 = std::string("") + CONNTRACK + UPDATE;
    std::string cmd2 = std::string("") + CONNTRACK + UPDATE;

    TwiceNatEntryValue value = m_twiceNatEntries[entry];

    cmd1 += (" -s " + entry.src_ip.to_string() + " -d " + entry.dst_ip.to_string() + " -t " + std::to_string(timeout) + REDIRECT_TO_DEV_NULL);
    cmd2 += (" -s " + value.translated_src_ip.to_string() + " -d " + value.translated_dst_ip.to_string() + " -t " + std::to_string(timeout) + REDIRECT_TO_DEV_NULL);
    /* We update the conntrack entry timeout with different set of values for the source and destination, because 
     * a single conntrack results in 2 twice nat entries and we don't know which twice nat entry in our cache is
     * inline with the fields of the conntrack entry */
    swss::exec(cmd1, res);
    swss::exec(cmd2, res);
...
void NatOrch::updateConnTrackTimeout(const TwiceNaptEntryKey &entry)
{
    ...
    cmd1 += (" -s " + entry.src_ip.to_string() + " -p " + prototype + " --orig-port-src " + to_string(entry.src_l4_port) + 
             " -d " + entry.dst_ip.to_string() + " --orig-port-dst " + std::to_string(entry.dst_l4_port) +  
             " -t " + std::to_string(timeout) + REDIRECT_TO_DEV_NULL);
    cmd2 += (" -s " + value.translated_src_ip.to_string() + " -p " + prototype + " --orig-port-src " + to_string(value.translated_src_l4_port) + 
             " -d " + value.translated_dst_ip.to_string() + " --orig-port-dst " + std::to_string(value.translated_dst_l4_port) + 
             " -t " + std::to_string(timeout) + REDIRECT_TO_DEV_NULL);
    /* We update the conntrack entry timeout with different set of values for the source ip/port and destination ip/port, because 
     * a single conntrack results in 2 twice napt entries and we don't know which twice napt entry in our cache is
     * inline with the fields of the conntrack entry */
    swss::exec(cmd1, res);
    swss::exec(cmd2, res);
...
void NatOrch::addConnTrackEntry(const TwiceNaptEntryKey &keyEntry)
{
    ...
    cmds += (" -n " + entry.translated_src_ip.to_string() + ":" + to_string(entry.translated_src_l4_port) + " -g " + entry.translated_dst_ip.to_string() + 
             ...
             " --dport " + to_string(keyEntry.dst_l4_port) + " -u ASSURED " + state);
    int ret = swss::exec(cmds, res);
