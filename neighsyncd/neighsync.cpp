...
NeighRestartAssit::NeighRestartAssit(RedisPipeline *pipelineAppDB,
  RedisPipeline *pipelineConfDB):
    ...
    m_cfgTable(pipelineConfDB, CFG_WARMSTART_TABLE_NAME, "|", false)
{
    m_startTime = time(NULL);
    m_appTableName = m_appTable.getTableName();
    
    vector<string> keys;
    m_cfgTable.getKeys(keys);
    assert(keys.size() <= 1);

    if (keys.size() == 0)
	SWSS_LOG_NOTICE("read configDB %s, keys not found",
	  m_cfgTable.getTableName().c_str());

    /* found in config DB */
    if (keys.size() == 1) 
    {
	vector<FieldValueTuple> fvs;
	if (m_cfgTable.get(keys[0], fvs))
	{
	    string s = "";
	    s = joinVectorString(fvs);
	    SWSS_LOG_NOTICE("read configDB %s %s: %s", m_cfgTable.getTableName().c_str(),
	      keys[0].c_str(), s.c_str());
		...
		if (fv.first == "warmstart_enable")
		{
		    if (!fv.second.empty())
		    {
			if(fv.second == "false" || fv.second == "FALSE" ||
			  fv.second == "0") 
...
string NeighRestartAssit::joinVectorString(vector<FieldValueTuple> fv)
{
    string s;
...
NeighRestartAssit::cache_state_t NeighRestartAssit::
  getCacheEntryState(const std::vector<FieldValueTuple> &fvVector)
{
...
void NeighRestartAssit::insertToMap(string key, vector<FieldValueTuple> fvVector,
  bool delete_key)
{
	...
	{
	    SWSS_LOG_NOTICE("%s, found key: %s, new value ",
	      m_appTableName.c_str(), key.c_str());
	...
	{
	    SWSS_LOG_INFO("%s, found key: %s, same value",
	      m_appTableName.c_str(), key.c_str());
    ...
    {
	SWSS_LOG_NOTICE("%s, not found key: %s, new",
	  m_appTableName.c_str(), key.c_str());
	FieldValueTuple state(CACHE_STATE, "UNKNOWN");
	fvVector.push_back(state);
	setCacheEntryState(fvVector, NEW);
